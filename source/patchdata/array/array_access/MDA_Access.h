/*
  File:		$URL: file:///home/gunney/csg/svnrepo/mda/branches/v_0_1/MDA_Access.h $
  Copyright:	(c) 2005 The Regents of the University of California
  Revision:	$Revision: 2305 $
  Modified:	$Date: 2008-07-21 15:24:43 -0700 (Mon, 21 Jul 2008) $
...
{
  dim_t i;
  if ( st )	{ for ( i=0; i<MDA_DIM; ++i ) { d_start[i] = st[i]; } }
  else		{ for ( i=0; i<MDA_DIM; ++i ) { d_start[i] = 0; } }
  if ( sz )	{ for ( i=0; i<MDA_DIM; ++i ) { d_size[i] = sz[i]; } }
...
{
  index_t i;
  if ( si )	{ for ( i=0; i<MDA_DIM; ++i ) d_start[i] = si[i]; }
  else		{ for ( i=0; i<MDA_DIM; ++i ) d_start[i] = 0; }
  if ( sf )	{ for ( i=0; i<MDA_DIM; ++i ) d_size[i] = 1 + sf[i] - d_start[i]; }
  else		{ for ( i=0; i<MDA_DIM; ++i ) d_size[i] = 0; }
  setDependentData();
  return;
}
...
void setInclusiveRange (
  /*! First valid indices (NULL for no change)	*/ const index_t first[MDA_DIM] ,
  /*! Final valid indices (NULL for no change)	*/ const index_t final[MDA_DIM] )
{
  if (first) for ( dim_t i=0; i<MDA_DIM; ++i ) d_start[i] = first[i];
  if (final) for ( dim_t i=0; i<MDA_DIM; ++i ) d_size[i] = final[i] - first[i] + 1;
  setDependentData();
  return;
}
...
friend std::ostream &operator<<( std::ostream &os, const MDA_IndexRange<MDA_DIM> &r ) {
  return r.streamPut(os);
}
friend std::istream &operator>>( std::istream &is, MDA_IndexRange<MDA_DIM> &r ) {
...
void setDependentData() {
  index_t i;
  for ( i=0; i<MDA_DIM; ++i ) d_stop[i] = d_start[i] + d_size[i];
}
...
*/
inline index_t offset ( const index_t i[MDA_DIM] ) const {
  int d;
  size_t o = i[MDA_DIM>0?MDA_DIM-1:0] - this->beg(MDA_DIM-1);
  for ( d=MDA_DIM-2; d>=0; --d ) o += ( i[d] - this->d_start[d] )*d_total_size[d+1];
  return o;
}
...
inline index_t offset (
                    index_t i0 , index_t i1 ) const {
  return
    (i0 - this->d_start[D0])*d_total_size[D1] +
    (i1 - this->d_start[D1]);
}
inline index_t offset (
                    index_t i0 , index_t i1 , index_t i2 ) const {
  return
    (i0 - this->d_start[D0])*d_total_size[D1] +
...
inline index_t offset (
                    index_t i0 , index_t i1 , index_t i2 , index_t i3 ) const {
  return
    (i0 - this->d_start[D0])*d_total_size[D1] +
    (i1 - this->d_start[D1])*d_total_size[D2] +
...
inline index_t offset ( const index_t i[MDA_DIM] ) const {
  ...
  for ( d=1; d<MDA_DIM; ++d ) o += ( i[d] - this->d_start[d] )*d_total_size[d-1];
  return o;
}
inline index_t offset (
                    index_t i0 ) const {
...
inline size_t reduce( index_t i, reduced_order_t &new_order ) const {
  ...
  return (i- this->d_start[MDA_Reduced_DIM])*d_total_size[MDA_DIM > 1 ? MDA_DIM-2 : 0];
...
inline void computeSizeDependentData() {
  ...
  for ( ; i<MDA_DIM; ++i ) {
    d_total_size[i] = this->d_size[i]*d_total_size[i-1];
    d_fixed_offset -= this->d_start[i]*d_total_size[i];
  }
  return;
}
/*!
...
MDA_Access<MDA_TYPE,OrderType::MDA_Reduced_DIM,typename OrderType::reduced_order_t> reduce( index_t i ) const {
  ...
  return MDA_Access<MDA_TYPE,OrderType::MDA_Reduced_DIM,typename OrderType::reduced_order_t>(
...
: MDA_Access< const MDA_TYPE, MDA_DIM,OrderType > ( (const MDA_TYPE*)(r), r.range() )
{
  return;
}
  /*!
