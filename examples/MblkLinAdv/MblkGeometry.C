   ...
   for (int k = pbox.lower(2); k <= pbox.upper(2)+1; k++) {
      for (int j = pbox.lower(1); j <= pbox.upper(1)+1; j++) {
         for (int i = pbox.lower(0); i <= pbox.upper(0)+1; i++) {
            hier::Index<NDIM> ic(i,j,k);
            pdat::NodeIndex<NDIM> node(ic,pdat::NodeIndex<NDIM>::LLL);
            hier::Index<NDIM> icm1(i-1,j-1,k-1);
            pdat::CellIndex<NDIM> cell(icm1);
   ...
   bool is_from_restart)
{
   ...
   for (nb = 0; nb < d_nblocks; nb++) {
      ...
      std::snprintf(block_name, sizeof(block_name), "rotation_%d", nb);
...
void MblkGeometry::getDx(const hier::Box<NDIM>& domain,
                         const int level_number,
                         double* dx)
...
void MblkGeometry::getDx(const int level_number,
                         double* dx)

{
   if (!d_metrics_set[level_number]) {
      TBOX_ERROR(d_object_name << ":metrics have not been set.\n"
                 << "Use the alternative 'getDx()' method call that "
                 << "takes in the domain." << endl);
   }
...
int MblkGeometry::getBlockRotation(const int block_number)
...
void MblkGeometry::setCartesianMetrics(const hier::Box<NDIM>& domain,
                                       const int level_number)
{
                                             ...
                                             const int block_number)
{
   ...
   for (pdat::NodeIterator<NDIM> ni(patch.getBox()); ni; ni++) {
      pdat::NodeIndex<NDIM> node = ni();
...
void MblkGeometry::setWedgeMetrics(const hier::Box<NDIM>& domain,
                                   const int level_number)
{
...
void MblkGeometry::setSShellMetrics(const hier::Box<NDIM>& domain,
                                    const int level_number)
{
   ...
   double nth  = ( domain.upper(1) - domain.lower(1) + 1 );
#if (NDIM == 3)
   double nphi = ( domain.upper(2) - domain.lower(2) + 1 );
#endif
